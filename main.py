...
def search(problem):
    root = tree_node.tree_node(problem.get_initial_state())     # initialize root of search tree with initial state
    ...
    while(frontier):                                        # run till frontier queue is empty
        ...
        unexplored_nodes = expand_node(node, problem, explored_nodes, frontier)     # call expand_node() to get new unexplored nodes
...
initial_state = [1, 2, 3, 0, 5, 6, 4, 7, 8]         # initial state of the problem
problem = Problem.Problem(initial_state)            # initialize problem object with initial state
solution_node = search(problem)                     # call search() to find the solution node
